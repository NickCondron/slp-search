...
macro_rules! regex_match_with {
    ($type: ident, $with: ident {
        $($variant_val: ident => $variant: ident),+ $(,)?
    }) => {
        impl RegexWithType for $type {
            type T = $with;
            fn try_match(t: &$with, s: &str) -> Option<$type> {
                match *t {
                    $( $with::$variant_val => $variant::try_match(s).map($type::$variant), )*
                    _ => None,
                }
            }
        }
...
regex_match_with!(Costume, External {
